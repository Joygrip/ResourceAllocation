...
async def create_demand_line(
    ...
    db: Session = Depends(get_db),
    current_user: CurrentUser = Depends(require_roles(UserRole.ADMIN, UserRole.PM)),
):
    """
...
async def create_supply_line(
    ...
    db: Session = Depends(get_db),
    current_user: CurrentUser = Depends(require_roles(UserRole.ADMIN, UserRole.RO)),
):
    """
